...
[`mcp-server/`](mcp-server/) wraps the research workflow as a callable tool for other agents, not just for a human running Claude Code — see [`mcp-server/README.md`](mcp-server/README.md) for setup and what's tested vs. not. [`MONETIZATION.md`](MONETIZATION.md) covers the distribution and pricing plan behind it.
